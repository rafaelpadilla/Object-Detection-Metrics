...
The motivation of this project is the lack of consensus used by different works and implementations concerning the evaluation metrics of the object detection problem. Although on-line competitions use their own metrics to evaluate the task of object detection, just some of them offer code snippets to calculate the accuracy of the detected objects. Among many implementations and documentations explaining such metrics, researchers still need to convert their ground truth and detected bounding boxes into different formats to obtain results for each metric.  
Consistent metrics to evaluate the performance of object detection techniques are necessary in order to have trustworthy benchmarking among different approaches.  
Different works produce their metrics using implementations offered by distinct tools. Sometimes results can be obtained differently if generated by other implementations. Thus, comparing performances using different implementations may produce biased results.  
This project aims to provide easy-to-use functions implementing the same metrics used by the the most popular competitions of object detection. Our implementation does not require modification of your detection model to complicated input formats, avoiding conversions to XML or JSON files. We simplified the input data (ground truth bounding boxes and detected bounding boxes) and gathered in a single project the evaluation of the object detection task with the main metrics used by the academia and challenges.

## Different competitions, different metrics  

**[PASCAL VOC challenge](http://host.robots.ox.ac.uk/pascal/VOC/)** offers a Matlab script in order to evaluate the quality of detected objects. A documentation explaining their criteria for object detection metrics can be accessed [here](http://host.robots.ox.ac.uk/pascal/VOC/voc2012/htmldoc/devkit_doc.html#SECTION00050000000000000000). Participants of the competition can use the script to measure the accuracy of their detections before submitting their results. The current metrics used by this VOC PASCAL object detection challange are the **Precision/Recall curve** and **Average Precision**.  
In order to use PASCAL VOC Matlab code, it is necessary to convert the detections in the required format by the matlab script (`VOCevaldet` function).  Even though projects such as [Faster-RCNN](https://github.com/rbgirshick/py-faster-rcnn) implement VOC PASCAL evaluation metrics, it is also necessary to convert the detected bounding boxes into a specific format. [Tensorflow](https://github.com/tensorflow/models/blob/master/research/object_detection/g3doc/evaluation_protocols.md) framework also has their PASCAL VOC metrics implementation.  
...
**Google Open Images Dataset V4 competition** also uses mean Average Precision (mAP) metric over the 500 classes to evaluate the object detection task. See [here](https://storage.googleapis.com/openimages/web/challenge.html) the documentation explaining their evaluation metric.  
...
Intersection Over Union is an evaluation metric used to measure the accuracy of an object detector given the ground truth bounding box ![](http://latex.codecogs.com/gif.latex?B_%7Bgt%7D) and a predicted bounding box ![](http://latex.codecogs.com/gif.latex?B_p) from your model. IOU is given by the overlapping area between the predicted bounding box and the ground truth bounding box divided by the area of union between the bounding boxes.  
...
The image belows shows the IOU between the ground truth bounding box (in green) and the detected bounding box (in red).
...
* **True Positive (TP)**: Correct detections. Detections with IOU ≥ _threshold_  
* **False Positive (FP)**: Wrong detections. Detection with IOU < _threshold_  
* **False Negative (FN)**: Ground truth not detected  
* **True Negative (TN)**: Does not apply. It would represent a corrected misdetection. In the detection object task there are many possible bounding boxes that should not be detected within an image. Thus, TN would be all possible bounding boxes that were corrrectly not detected. That's why it is not used by the metrics.
...
Recall is ability of a model to find all the relevant cases. It is the percentage of true positive detected among all relevant ground truths and is given by:
...
The precision x recall curve is a good way to evaluate the performance of an object detector as the confidence is changed. There is a curve for each object class. An object detector given a particular class is considered good if its prediction stays high as recall increases, which means that if you vary the confidence threshold the precision and recall will still be high. Another way to identify a good object detector is to look for a detector that can identify only relevant objects (0 False Positives = high precision), finding all ground truth objects (0 False Negatives = high recall). A poor object detector needs to increase the number of detected objects (increasing False Positives = lower precision) in order to retrieve all ground truth objects (high recall). These two arguments explain the shape of the Precision x Recall curve: it starts with high precision values, decreasing as recal increases.
This kind of curve is used by the VOC PASCAL 2012 challenge and is available in our implementation.  

### Average Precision

Another way to compare the performance of object detectors is to calculate the area under the curve (AUC) of the Precision x Recall curve. As AP are often zigzag curves frequently going up and down, having different curves in the same plot usually is not an easy task, once they tend to cross each other much frequently. That's why Average Precision (AP), a numerical metric, can also help us compare different detectors. In practice AP is the precision averaged across all recall values between 0 and 1. 

VOC PASCAL 2012 challenge uses the *interpolated average precision*. They try to summarize the shape of the Precision x Recall curve by averaging the precision at a set of eleven equally spaced recall levels [0, 0.1, 0.2, ... , 1]:
...
There is a total of 7 images with 15 ground truth objects representented by the greend bounding boxes and 24 detected objects represented by the red bounding boxes. Each detected object is represented by a letter (A,B,...,Y) and has its confidence level.  
The following table shows the bounding boxes with their corresponding confidences. The last column identifies if each detection is TP or FP, considering a TP if IOU ![](http://latex.codecogs.com/gif.latex?%5Cgeq) 30%, otherwise it is considered a FP (By looking at the images above we can roughly tell if the detections are TP or FP).
...
In some images there are more than one detection overlapping the ground truth. In those cases, it is taken the detection with the highest IOU, discarding the other detections. This rule is also applied by the VOC PASCAL 2012 metric: "e.g. 5 detections of a single object is counted as 1 correct detection and 4 false detections”.
...
| Image 4 |	O	| 14% | 0 | 1 | 7 | 17 | 0.2916 | 0.4666 |
 ...
 Plotting those values we have the following *Precision x Recall curve*:
...
The Average Precision (AP) is the AUC obtained by the interpolated precision. The intention is to reduce the impact of the wiggles in the Precision x Recall curve.
